...
// ✅ Раздаём ассеты Mini App (css/js/images)
app.use("/miniapp-assets", express.static(miniappDir, { maxAge: "1h" }));

// ✅ Mini App: всегда отдаём index.html
...
async function requireTelegramAuth(req, res, next) {
  ...
  if (userId && ENABLE_CHANNEL_GATE) {
    try {
      ...
      // Если проверка временно не работает — не блокируем (как и в боте)
    }
  }
...
// ✅ ВАЖНО: не сбрасываем кредиты каждый раз!
function ensureUserFromTg(tgUser) {
  ...
  if (!existing) {
    ...
    // Обновим только имя/ник (не трогаем credits!)
...
// ✅ Telegram Stars invoice link (Mini App)
...
app.post("/api/generate", requireTelegramAuth, async (req, res) => {
  ...
  try {
    const task = await createMysticTask({
      ...
      prompt: prompt.trim(), // ✅ строго твой текст
    ...
    // Быстрое ожидание результата (до 60 сек)
